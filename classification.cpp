...
void test_proposal_f1_score_vs_iou(string directory){
  ...
  for(int i = 1; i <= testset_size; i++){
    ...
    vector<Rect>* p_regions = region_proposal(&(image_and_gt_boxes->first));

    // get IoU and F1-scores for each IOU threshold, for the current image
    vector<pair<float, float> >* IoU_F1 = compute_F1_vs_IoU(image_and_gt_boxes->second, p_regions);   
...
void dump_IoU_F1(vector<pair<float, float> >* overall_IoU_F1){
  ...
  string f1_score = "";
  ofstream outfile;
  outfile.open("./myexperiments/iou_vs_f1scores.txt");
